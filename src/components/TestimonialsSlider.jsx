...
function TestimonialsSlider() {
  ...
  const settings = {
    ...
    speed: 10000, // The duration of the transition between slides in milliseconds. Here, 2000ms means it takes 2 seconds to complete a slide transition, providing a smooth and slower scrolling effect.
    ...
    autoplaySpeed: 10000, // Time delay in milliseconds before automatically transitioning to the next slide. Set to 15000ms (15 seconds) to allow the user ample time to read each testimonial.
  ...
  const testimonials = [
    ...
    },
    {
      name: 'Josh Ray',
      quote:
        'Adam Nuccios vision and decisiveness simplify tasks and processes, allowing the team to focus on important milestones. His attention to detail creates an environment dedicated to precision and excellence in every facet.',
      ...
      <Slider {...settings}>
        {testimonials.map((testimonial, index) => (
          <div key={index} className='testimonial'>
            ...
            {/* Display the role/title of the person */}
          </div>
        ))}
      </Slider>
    </div>
